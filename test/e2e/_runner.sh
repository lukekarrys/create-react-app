...
function print_help {
	...
	echo "  --dry-run		             	immediately exit the docker container [${dry_run}]"
...
function get_test_command {
	local setup_command="source \$(dirname "\$0")/_setup.sh"
	local test_command=""
	...
	for test_suite in "${test_suites[@]}"; do
		...
		if [[ "$test_suite" == "old-node" ]]; then
			if [[ "$run_all_suites" == "true" ]]; then
				...
				test_command="${setup_command} && ${command}"
				break
			fi
		fi

		test_command="${test_command:-true} && ${setup_command} && ${command}"
	done
